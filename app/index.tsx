...
export default function App() {
    ...
    const openQrForm = (prod: ItemEstoque | null = null) => {
        if (prod) {
            ...
            setProdPrice(prod.price ? String(prod.price).replace('.', ',') : '');
        ...
        try {
            ...
            Alert.alert("Restaurar da Nuvem", "Isso irá substituir todos os dados atuais pelos da planilha. Continuar?", [
                { text: "Cancelar", onPress: () => setIsSyncing(false) },
                {
                    text: "Restaurar", onPress: async () => {
                        try {
                            // 0. Fazer backup local das imagens dos produtos antes de apagar
                            const currentInventory = await DatabaseService.getInventory();
                            const imageBackupMap: Record<string, string> = {};
                            currentInventory.forEach(item => {
                                if (item.image) {
                                    imageBackupMap[item.id] = item.image;
                                }
                            });
                            ...
                            if (data.sales) {
                                for (const s of data.sales) {
                                    const products = data.sale_items.filter((i: any) => i.sale_id === s.id);
                                    let installments = data.installments.filter((i: any) => i.sale_id === s.id);

                                    // FIX / FALLBACK: Se o Google Script retornou as parcelas com valor 0, calcular pelo total dos produtos
                                    const sumInst = installments.reduce((acc: any, inst: any) => acc + parseFloat(inst.value || 0), 0);
                                    if (sumInst === 0 && installments.length > 0) {
                                        const totalValueCalc = products.reduce((acc: any, p: any) => {
                                        ...
                                        if (totalValueCalc > 0) {
                                            const valPerInst = totalValueCalc / installments.length;
